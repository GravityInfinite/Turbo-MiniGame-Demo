...
import turbo from "./js/utils/turbo.min"
...
function initDraw() {
  ...
  const rightStartLeftTop = (screenWidth / 2 - buttonWidth) / 2 + screenWidth / 2
  ...
  for (let i = 0; i < 7; i++) {
    ctx.fillStyle = '#67c23a'
    ctx.fillRect(leftStartLeftTop, 90 + (buttonHeight + marginTop) * i, buttonWidth, buttonHeight)
    ctx.fillRect(rightStartLeftTop, 90 + (buttonHeight + marginTop) * i, buttonWidth, buttonHeight)
    buttonLocation.push(...[
    ...
    ctx.fillText(eventList[i * 2], getTextLeftStart(leftWidth, 'left'), 110 + (buttonHeight + marginTop) * i)
    ctx.fillText(eventList[i * 2 + 1], getTextLeftStart(rightWidth, 'right'), 110 + (buttonHeight + marginTop) * i)
  }
...
canvas.addEventListener('touchstart', (e) => {
  const eventIndex = buttonLocation.findIndex((item) => {
    const x = e.touches[0].clientX,
      ...
      width = item[0],
      height = item[1]
    return width + buttonWidth > x && height + buttonHeight > y && (height < y) && (width < x)
  })
  if (eventIndex === -1) {
    return
  }
